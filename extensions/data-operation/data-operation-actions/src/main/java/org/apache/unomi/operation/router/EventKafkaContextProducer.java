...
public class EventKafkaContextProducer implements SynchronousBundleListener, EventProducer {
    ...
    public void initCamelContext() throws Exception {
        ...
        logger.info("KAFKA Config " + uriBuilder.toString());
        try {
            camelContext.addRoutes(new RouteBuilder() {
                ...
                public void configure() throws Exception {
                    ...
                    from("direct:kafkaRoute")
                            //.marshal(objectMapper)
                            .log("Send to DataOperation: ${body}")
                            .setHeader("kafkaTopic", simple("${body.eventType}"))
                            .marshal(objectMapper)
                            .toD("kafka:localhost:9092?topic=${header.kafkaTopic}")
